...
class _TimelineBodyState extends State<TimelineBody> {
  ...
  Future<void> updateTripStatus(int newStatus) async {
    ...
    if (userTripStatus == 1 && newStatus == 0) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('Không thể quay lại trạng thái đang áp dụng sau khi đã hoàn thành.'
    ...
    try {
      ...
      if (userTripDocId != null) {
        ...
        await Future.wait([
          FirebaseFirestore.instance.collection('user_trip')
              .doc(userTripDocId).update(updateData),

          usersCol.doc(currentUserId).collection('selected_trips')
              .doc(widget.tripId).set(updateData, SetOptions(merge: true))
        ]);
      } else {
        // Tạo mới và đồng bộ dữ liệu
        final docRef = await FirebaseFirestore.instance.collection('user_trip').add({
        ...
        await usersCol.doc(currentUserId).collection('selected_trips')
            .doc(widget.tripId).set(updateData, SetOptions(merge: true));
  ...
  Future<void> checkAllActivitiesCompleted() async {
    ...
    try {
      ...
      for (var tlDoc in timelinesSnap.docs) {
        final scheduleSnap = await tlDoc.reference
            .collection('schedule')
            .get();
      ...
      print('Hoàn thành: $completedCount / $totalActivities hoạt động của user $userId');
    } catch (e) {
          ...
          timelines.docs.map((timeline) async {
            ...
            await Future.wait(
                schedules.docs.map((schedule) =>
                    schedule.reference.update({'isCompleted': false})
                )
            );
          })
      );
  ...
  Future<void> createNewUserTrip() async {
    ...
    try {
      ...
      for (var timelineDoc in timelinesSnapshot.docs) {
        final timelinePath = _userTripRef!.collection('timelines').doc(timelineDoc.id);
